...
class ChainParses extends ChainParseBuilder {
  var chainParses = HashMap[(NonTerminal, NonTerminal), ParseTree]()

  private def addChildToBottom(parseTree: ParseTree, child: NonTerminal): ParseTree = {
    parseTree match {
      case ParseTreeNode(name, children) => ParseTreeNode(name, ListBuffer(addChildToBottom(children.head, child)))  // These parses are all chain parses, so every node will have at most one child
  ...
  def newDerivation(from: NonTerminal, to: NonTerminal, rule: Rule): Unit = {
    ...
    if (rule.getLeft() == from && rule.getRight().head == to){    // When the method is called, the rule should always be a chain rule otherwise it makes no sense (since there are no lambdas left)
      ...
      newParseTree = addChildToBottom(existingParseTreeToHere, to)
    }
